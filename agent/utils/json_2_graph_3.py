...
def visualize_opsearch_results(
    ...
    def _determine_chart_type(query_type: str, data_structure: Dict) -> str:
        ...
        # ===== 自动识别：双特征风险热力图（必须最先判断）=====
        if query_type == "distribution":
            buckets = data_structure.get("buckets", [])
            if len(buckets) >= 2:
                first = buckets[0]
                # 情况1：dimensions 路径
                dims = query_json.get("query", {}).get("config", {}).get("dimensions", [])
                if len(dims) == 2:
                    for dk in dims:
                        for sub in first.get("dimensions", {}).get(dk, {}).get("buckets", []):
                            met = sub.get("metrics", {})
                            if ("positive" in met) or (met.get("percentage") is not None):
                                return "risk_heatmap"
                # 情况2：三级 terms 桶，只要末级出现过 key=1 即视为可画风险热力图
                for sub in first.get("sub_aggregations", {}).get("buckets", []):
                    if any(leaf.get("key") == 1 for leaf in sub.get("sub_aggregations", {}).get("buckets", [])):
                        return "risk_heatmap"
    ...
    def _plot_risk_heatmap(data: Dict, query_json: Dict, output_path: str) -> str:
        """
        自动双特征风险热力图
        依赖：query.config.dimensions 长度=2，且 buckets 里能提取
              (dimension1, dimension2, positive, total) 四元组
        """
        import seaborn as sns
        cfg = query_json.get("query", {}).get("config", {})
        dims = cfg.get("dimensions", [])
        if len(dims) != 2:
            raise ValueError("auto risk_heatmap needs exactly 2 dimensions")

        # 1. 暴力拍平成 (d1, d2, positive, total)
        records = []
        for b in data.get("buckets", []):
            d1_buckets = b.get("dimensions", {}).get(dims[0], {}).get("buckets", [])
            d2_buckets = b.get("dimensions", {}).get(dims[1], {}).get("buckets", [])
            for b1 in d1_buckets:
                for b2 in d2_buckets:
                    pos = b1.get("metrics", {}).get("positive") or \
                          b1.get("metrics", {}).get("percentage", 0) * b1.get("doc_count", 0) / 100
                    tot = b1.get("doc_count") or b1.get("metrics", {}).get("count", 0)
                    records.append((b1.get("key"), b2.get("key"), int(pos), int(tot)))

        df = pd.DataFrame(records, columns=["g1", "g2", "positive", "total"])
        df["risk_pct"] = df["positive"] / df["total"] * 100
        heat = df.pivot_table(values="risk_pct", index="g1", columns="g2", fill_value=0)

        # 2. 画图
        fig, ax = plt.subplots(figsize=(6, 4))
        sns.heatmap(heat, annot=True, fmt=".1f", cmap="RdYlGn_r",
                    vmin=0, vmax=100, cbar_kws={"label": "Risk %"}, ax=ax)
        ax.set_title(f"Risk %  ({dims[0]} × {dims[1]})")
    ...
    try:
        ...
        if chart_type == "single_metric_summary" or chart_type == "multi_metric_summary":
            ...
            return _plot_risk_heatmap(result_json, query_json, output_path)
...
if __name__ == "__main__":
    ...
    # =============== 自动风险热力图 测试 ===============
    print("\n[Test] Auto risk-heatmap: 双维度 + 阳性/总计")
    query_heatmap = {
        "query": {
            "type": "distribution",
            "config": {
        ...
        "buckets": [
            {
                ...
                "dimensions": {
                    "age_group": {
                        "buckets": [
                            {"key": "20-40", "doc_count": 1000, "metrics": {"count": 1000, "positive": 120}}
                        ]
                    },
                    "gender": {
                        "buckets": [
                            {"key": "Male",   "doc_count": 600, "metrics": {"count": 600, "positive": 84}},
                            {"key": "Female", "doc_count": 400, "metrics": {"count": 400, "positive": 36}}
                        ]
                    }
                }
            },
            {
                ...
                "dimensions": {
                    "age_group": {
                        "buckets": [
                            {"key": "40-60", "doc_count": 800, "metrics": {"count": 800, "positive": 160}}
                        ]
                    },
                    "gender": {
                        "buckets": [
                            {"key": "Male",   "doc_count": 480, "metrics": {"count": 480, "positive": 96}},
                            {"key": "Female", "doc_count": 320, "metrics": {"count": 320, "positive": 64}}
                        ]
                    }
                }
            }
    ...
    heat_path = visualize_opsearch_results(query_heatmap, result_heatmap, "./examples")
    print(f"Auto risk-heatmap saved to: {heat_path}")